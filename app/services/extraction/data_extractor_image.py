...
def crop_and_save_images(image, combined_lines, page_num, output_dir,color_name,document_number,json_path):
    ...
    if os.path.exists(json_path):
        ...
        json_data = {"change_links": []}  # Create an empty dictionary if the file doesn't exist
    ...
    uploaded_urls = []
        # Ensure the change_links key exists in the JSON data
    if "change_links" not in json_data:
        json_data["change_links"] = []
    ...
    for i, (x1, y1, x2, y2) in enumerate(combined_lines):
       ...
       # Save locally 
        ...
        # Add the uploaded URL to the change_links list in JSON data
        json_data["change_links"].append(uploaded_url)
